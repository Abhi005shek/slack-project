      ...
      page: (
        await Promise.all(
          results.page.map(async (message) => {
            ...
            const rawReactions = await populateReactions(ctx, message._id);
            const thread = await populateThread(ctx, message._id);
            const image = message.image
              ? await ctx.storage.getUrl(message.image)
              : undefined;

            const reactionsWithCounts = rawReactions.map((reaction) => ({
              ...reaction,
              count: rawReactions.filter((r) => r.value === reaction.value)
                .length,
            }));
...
export const create = mutation({
  ...
  async handler(ctx, args) {
    ...
    // Only possible when we are replying in a thread in 1-on-1 conversations
    if (!args.conversationId && !args.channelId && args.parentMessageId) {
      const parentMessage = await ctx.db.get(args.parentMessageId);
